...
const ppc = {
  id: 'test-ppc',
  subjects: [
    { code: 'MAT01', name: 'Cálculo I', workloadHours: 60, prerequisites: [], corequisites: [], equivalents: ['MAT01-OLD'], minWorkloadHours: null },
    { code: 'MAT02', name: 'Cálculo II', workloadHours: 60, prerequisites: ['MAT01'], corequisites: [], equivalents: [], minWorkloadHours: null },
    { code: 'ELE01', name: 'Circuitos I', workloadHours: 60, prerequisites: ['MAT01'], corequisites: ['ELE02'], equivalents: [], minWorkloadHours: null },
    { code: 'ELE02', name: 'Lab. Circuitos I', workloadHours: 30, prerequisites: [], corequisites: [], equivalents: [], minWorkloadHours: null },
    { code: 'EST01', name: 'Estágio', workloadHours: 200, prerequisites: [], corequisites: [], equivalents: [], minWorkloadHours: 100 },
  ],
};

function section(subjectCode, turma, sessions = []) {
  return { kind: 'offering', subjectCode, turma, sessions, failed: false, audit: false, id: `${subjectCode}-${turma}` };
}
...
describe('evaluatePlan', () => {
  it('marks a Subject fulfilled by a non-failed Section as satisfying a later prerequisite', () => {
    const profile = baseProfile({
      semesters: [{ sections: [section('MAT01', '01')] }, { sections: [section('MAT02', '01')] }],
    });
  ...
  it('flags an Unmet Requisite when the prerequisite was never planned', () => {
    const profile = baseProfile({ semesters: [{ sections: [section('MAT02', '01')] }] });
    const result = evaluatePlan(profile, ppc, {});
  ...
  it('satisfies a co-requisite planned in the same semester', () => {
    const profile = baseProfile({
      semesters: [{ sections: [section('MAT01', '01')] }, { sections: [section('ELE01', '01'), section('ELE02', '01')] }],
    });
    const result = evaluatePlan(profile, ppc, {});
    const ele01 = result.semesters[1].sections.find((s) => s.subjectCode === 'ELE01');
  ...
  it('flags an Unmet Requisite when the co-requisite is missing from the same semester', () => {
    const profile = baseProfile({
      semesters: [{ sections: [section('MAT01', '01')] }, { sections: [section('ELE01', '01')] }],
    });
    const result = evaluatePlan(profile, ppc, {});
    const ele01 = result.semesters[1].sections.find((s) => s.subjectCode === 'ELE01');
  ...
  it('resolves an equivalent code to its canonical Subject for fulfillment', () => {
    const profile = baseProfile({
      semesters: [{ sections: [section('MAT01-OLD', '01')] }, { sections: [section('MAT02', '01')] }],
    });
    const result = evaluatePlan(profile, ppc, {});
  ...
  it('evaluates a minWorkloadHours threshold requisite against fulfilled workload', () => {
    const profile = baseProfile({
      semesters: [{ sections: [section('MAT01', '01')] }, { sections: [section('EST01', '01')] }],
    });
    const result = evaluatePlan(profile, ppc, {});
  ...
  it('flags a Duplicate Subject when two Sections in the same semester fulfill the same Subject', () => {
    const profile = baseProfile({ semesters: [{ sections: [section('MAT01', '01'), section('MAT01', '02')] }] });
    const result = evaluatePlan(profile, ppc, {});
    expect(result.semesters[0].sections.every((s) => s.signals.duplicateSubject)).toBe(true);
  ...
  it('flags a Schedule Conflict for overlapping sessions in the same semester', () => {
    const offerings = {
      subjects: [
        { code: 'MAT01', sections: [{ turma: '01', sessions: [{ day: 'Seg', startTime: '08:00', endTime: '10:00' }] }] },
        { code: 'ELE02', sections: [{ turma: '01', sessions: [{ day: 'Seg', startTime: '09:00', endTime: '11:00' }] }] },
      ],
    };
    const profile = baseProfile({ semesters: [{ sections: [section('MAT01', '01'), section('ELE02', '01')] }] });
  ...
  it('flags a Redundant Enrollment when a Subject is already fulfilled without an open Audit Mark', () => {
    ...
    const result = evaluatePlan(profile, ppc, {});
    expect(result.semesters[1].sections[0].signals.redundantEnrollment).toBe(true);
  });
  ...
  it('suppresses the Redundant Enrollment flag when the fulfillment carries an open Audit Mark', () => {
    ...
    const result = evaluatePlan(profile, ppc, {});
    expect(result.semesters[1].sections[0].signals.redundantEnrollment).toBe(false);
  });

  it('a Failed Mark still satisfies a same-semester co-requisite', () => {
    const profile = baseProfile({
      semesters: [{ sections: [section('MAT01', '01')] }, { sections: [section('ELE01', '01'), { ...section('ELE02', '01'), failed: true }] }],
    });
    const result = evaluatePlan(profile, ppc, {});
    const ele01 = result.semesters[1].sections.find((s) => s.subjectCode === 'ELE01');
    expect(ele01.signals.unmetRequisite).toBe(false);
