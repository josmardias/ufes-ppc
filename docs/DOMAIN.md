...
> **Note:** Electives are described here for context only. In this tool, planning is restricted to Subjects of the Student's own PPC — a real-world elective can be represented as a Custom Section (see below). A Subject with no known classification is treated as Required. Required and Optional Subjects are planned through separate flows (see `USE_CASES.md`, UC-11/UC-12 and UC-27), and the Student may mark an Optional Subject as **hidden** ("not interested") so it stops being offered when planning — a per-profile, reversible preference that affects listings only (UC-28).
...
A Student's Planned Semesters form an ordered sequence of positions in the program — the student's 5th, 6th, 7th semester, and so on. The sequence starts at the position right after the profile's completed-semester count (see Student): the completed semesters below it are represented as Credit Entries, not as Planned Semesters. This sequence reflects the student's own progression and is independent of the Suggested Semesters assigned to Subjects in the PPC.
...
Each Planned Semester is associated with a **Year Semester label** (e.g. "2024/1"), derived from the semester's position in the sequence and the Student's ingress information (provided at profile creation) — the completed-semester offset keeps positions, and therefore labels, aligned with the calendar.
