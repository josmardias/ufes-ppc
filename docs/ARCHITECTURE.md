...
A snapshot is **year-agnostic**: it represents what a typical Year Semester looks like for a course, built from the most recent source semester available (e.g. sources from 2026/1 fill the Year Semester 1 snapshot). When newer source PDFs are added, they **replace** the corresponding snapshot — the dataset is a snapshot, not an archive. Because snapshots are replaced, a planned Section can stop matching the current snapshot; the plan still renders it from its embedded copy (see `ProfileRecord`) and the divergence is surfaced as the **Offering Mismatch** warning (see `DOMAIN.md`).
...
**One documented exception** to the determinism rule: the migration that backfills the embedded planning copy (`sessions`, `targetCourseId`/`targetCourseName`) onto planned offering Sections created before copies existed reads the **current** Offerings snapshot — deliberately, accepted because the user base at the time was small and directly reachable. A Section that does not resolve at migration time gets an empty copy (no sessions, no target course) and simply surfaces as an Offering Mismatch (see `DOMAIN.md`), which is the truth.
...
`name`, `ingressYear`, `ingressYearSemester`, `shift`, `ppcId`, and `completedSemesters` are provided by the user at profile creation (UC-02) and are never null — the PPC is chosen there via the course → PPC cascade, and legacy profiles are backfilled by migration (see Migrations above). `courseId` is the official UFES course code, copied from the chosen PPC dataset when the profile is created. The profile thus records course (`courseId`), curriculum version (`ppcId`), and ingress (`ingressYear` + `ingressYearSemester`) as separate facts: all PPC versions of a course share the same `courseId`, which is what Section target course ids are matched against (see Offerings dataset). Apart from `name` (UC-08), profile data is **immutable after creation**: to change it, the user deletes the profile and creates a new one (UC-05, UC-02; see `DOMAIN.md`, Student).

`completedSemesters` is the last completed semester recorded at creation (UC-02): creation seeds a Credit Entry for each Required Subject suggested up to it, and the plan's Planned Semesters start at position `completedSemesters + 1` — the offset feeds position/label derivation and UC-14's deletion floor.
...
Each `PlannedSection` carries its own flags: `failed: boolean` (Failed Mark, see `DOMAIN.md`) and `audit: boolean` (Audit Mark). An `offering` Section stores the subject code it was offered under, the `turma`, and an **embedded planning copy** captured when it was added: `sessions` and `targetCourseId`/`targetCourseName` — never the professor, which is looked up live from the snapshot while the Section still matches, as display enrichment only. Rendering and Schedule Conflict detection use the embedded copy exclusively; the snapshot is compared against the copy (subject code + turma + session equality) only to derive the **Offering Mismatch** warning (see `DOMAIN.md`). When the offered code is an equivalent, resolution to the PPC subject happens at evaluation time and is never persisted. Applied Custom Sections are independent copies embedded in the semester — they do not reference the catalog. Deleting a Planned Semester deletes its contents with it.

`creditEntries` stores the student's Credit Entries (see `DOMAIN.md`): profile-level and timeless, each referencing a Subject code of the current PPC, with an `audit: boolean` flag for the Audit Mark. In the planner they surface as the **Completed (Concluídos)** pre-semester entry (UC-09), where the Course Curriculum is presented as a checklist — checking a Subject creates its Credit Entry, unchecking removes it — and Audit Marks are toggled (UC-15, UC-20, UC-21).
...
Planning signals — Unmet Requisites, Schedule Conflicts, Duplicate Subjects, Redundant Enrollments, Offering Mismatches — and the open/closed state of Audit Marks are never persisted: they are recomputed from the semester sequence by `src/domain` functions on every evaluation (see `DOMAIN.md`, Planned Semester).
